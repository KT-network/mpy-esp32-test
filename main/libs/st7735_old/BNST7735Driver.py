from libs.st7735.ST7735 import TFT
from machine import SPI, Pin
from framebuf import FrameBuffer, RGB565
import time
from math import sqrt
import libs.st7735.FileUtil as fu

# 屏幕原始方向尺寸（旋转前）
ScreenSize = (128, 160)
# 旋转后的偏移量
offset = bytearray([0, 0])
...
class BNST7735Driver:
    def __init__(self, scl, sda, res, dc, cs,blk):
        self.bufData = bytearray(ScreenSize[0] * ScreenSize[1] * 2)
        self.buf = FrameBuffer(self.bufData, ScreenSize[1], ScreenSize[0], RGB565)
        self.spi = SPI(2, baudrate=20000000, polarity=0, phase=0, sck=Pin(scl), mosi=Pin(sda), miso=Pin(blk))
